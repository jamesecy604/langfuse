...
export type FetchTracesTablePropsByUser = {
  select: "count" | "rows" | "metrics";
  userId: string;
  filter: FilterState;
  searchQuery?: string;
...
export const getTracesTableCountByUser = async (props: {
  userId: string;
  filter: FilterState;
  searchQuery?: string;
  orderBy?: OrderByState;
  ...
  const countRows = await getTracesTableGenericByUser<{ count: string }>({
    select: "count",
    tags: { kind: "count" },
    ...props,
  });
...
export const getTracesTableMetricsByUser = async (props: {
  userId: string;
  filter: FilterState;
  searchQuery?: string;
  orderBy?: OrderByState;
  ...
  const countRows =
    await getTracesTableGenericByUser<TracesTableMetricsClickhouseReturnType>({
      select: "metrics",
      tags: { kind: "analytic" },
      ...props,
    });
...
export const getTracesTableByUser = async (
  userId: string,
  filter: FilterState,
  searchQuery?: string,
  orderBy?: OrderByState,
  ...
  const rows = await getTracesTableGenericByUser<TracesTableReturnType>({
    select: "rows",
    tags: { kind: "list" },
    userId,
    filter,
...
const getTracesTableGenericByUser = async <T>(
  props: FetchTracesTablePropsByUser,
) => {
  const { select, userId, filter, orderBy, limit, page, searchQuery } = props;
  ...
  const { tracesFilter, scoresFilter, observationsFilter } =
    getUserIdDefaultFilter(userId, { tracesPrefix: "t" });
  ...
  const params = {
    ...
    offset:
      limit !== undefined && page !== undefined ? Number(limit * page) : 0,
  ...
  const res = await queryClickhouse<T>({
    ...
    params: {
      ...params,
      ...processParams(tracesFilterRes.params),
      ...processParams(observationFilterRes.params),
      ...processParams(scoresFilterRes.params),
      ...processParams(search.params),
    },
    tags: {
      ...(props.tags ?? {}),
      feature: "tracing",
